...
#define  RC_Ueff 10.0 /*unit:A*/          /*cutoff for rdf*/
#define  RC_Sel_in 0.0 /*unit:A*/          /*cutoff for inner shell*/
#define  RC_Sel_out 3.0 /*unit:A*/          /*cutoff for outer shell*/
#define  RC_Adf 3.0 /*unit:A*/               /*cutoff for adf(angle potential)*/
#define  ZH +0.417
#define  ZO -0.834
...
double Urdf(double g[][4], int nbin, double r, int isite, int tsite)  /*input a distance and output corresponding urdf */
{
	int i=0;
	for(i=0;i<nbin;i++)
		if(g[i][0]>r)
...
int main(void)
{
	 ...
	 */
	int N = 4913, nmove = 1000, fout = 100, k = 1000,oren = 50, acc_count = 0, natom = 0, num_cps = 0, num_shell = 0, nbin_rdf = 1600, nbin_adf = 750;  /*k:number of positions for O, oren:number of positions for H*/
	int n, j,m, jbook, jchoose, nsel, imol=0, isite=0, terminate = 0;         /*loop control*/
	...
	double L = 5.27731, T = 300.0, beta, coordinate[N][3][4], temp[N][3][4], center[3],select[k][3][4], selectH[oren][4],box[3],cps[N][3][4],temp_cps[N][3][4], g[nbin_rdf][4], a[nbin_adf+1][2];
	...
	char directory_in[100] = "/Users/Ziwei/Desktop/INPUT/", open_name[100] ="w100.gro", directory_out[100] = "/Users/Ziwei/Desktop/Aug_04_001/";
	FILE *fp_log;
		...
		for(imol=0;imol<nsel;imol++)
		{
			...
			for(j=0;j<k;j++)             /*generate k trials for molecule(oxygen)*/
			{
				if(imol < num_shell)  /*which means put a new molecule in a outer shell based on the bias potential*/
				{
					...
					if(wO[j]-0 < 0.000000000000001)    /*if wO is too small, do not need to calculate hydrogen, discard it directly*/
						continue;
					else
					{
						sumwH1 = 0;
				...
				fprintf(fp_log,"w_new[%d]:%.5f\n",j,w[j]);
			}

			jchoose = Select(w,sumw[imol]);
			wn *= sumw[imol];
			fprintf(fp_log,"w_new_selected[%d]:%.5f\n",jchoose,w[jchoose]);
//			printf("select:w[%d] = %.10f\n",jchoose,w[jchoose]);

			if(imol < num_shell)
				ueff_n *= w[jchoose];

			if(sumw[imol]-0 < 0.0000000001)  /*all the Boltzman weight is zero, which means there is no enough room for the left molecules*/
			{
				terminate = 1;
				fprintf(fp_log,"sum[%d] is too small, TERMINATE this trial move\n",imol);
				printf("sum[%d] is too small, TERMINATE this trial move\n",imol);
				break;
			}
...
/*from here, start to calculate the old configuration*/
			...
			for(j=0;j<k;j++)          /*generate k-1 trials*/
			{
				if(imol < num_shell)   /*which means put a new molecule in a outer shell based on the bias potential*/
				{
					...
					if(j != 0)
					{
						...
						if(wO[j]-0 < 0.000000000000001)    /*if wO is too small, we do not need to calculate hydrogen*/
							continue;
						else
						{
						sumwH1 = 0;
					...
					else /*Boltzman weight for the current old configuration*/
					{
						w[j] *=  Ueff_cav(temp_cps, g, a, nbin_rdf, nbin_adf, index_sel_cps[imol], 0, num_cps, L, index_sel_cps, nsel,imol);
						w[j] *=  Ueff_cav(temp_cps, g, a, nbin_rdf, nbin_adf, index_sel_cps[imol], 1, num_cps, L, index_sel_cps, nsel,imol);
						w[j] *=  Ueff_cav(temp_cps, g, a, nbin_rdf, nbin_adf, index_sel_cps[imol], 2, num_cps, L, index_sel_cps, nsel,imol);
						ueff_o *= w[0];
					}
				}
				else  /*which means put a new molecule in a inner shell based on the actual potential*/
		...
		if(num_shell > 0 && terminate == 0)  /*acc for outer shell*/
		{
			uact_n = 0;
			uact_o = 0;
//			Tnew = 1;
...
//					Told *= Ueff_cav(temp_cps, g, a, nbin_rdf, nbin_adf, index_sel_cps[imol], isite, num_cps, L,index_sel_cps, nsel, imol);
		...
		else if(terminate == 0)  /*acc for inner shell*/
			acc = wn/wo;
		else
			acc = 0;
