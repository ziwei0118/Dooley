//Dooley_v2.1
//insertion of water in cavity are all based on rdf potential
...
int serach_number(int n,int array[],int len)       /*used for check a int number is in a array or not,and return the index of the number in tha array*/
{
	int i,judge = -1;
	for(i=0;i<len;i++)
	{
...
void output_rdf(double coordinate[][3][4], int N, double L, char directory_address[])
{
	...
	double delg = 0.001 ,r,vb,nid_OO,nid_HH,nid_OH;
...
int main(void)
{
	...
	int N = 4913, nmove = 1000, fout = 10, k = 500, oren = 10, acc_count = 0, natom = 0, nbin = 1600;
	int n, j,m, jbook, jchoose, nsel, imol=0, isite=0;         /*loop control*/
	int index_sel[N];
	double L = 5.27731, T = 300.0, beta, coordinate[N][3][4], temp[N][3][4], center[3],select[k][3][4], selectH[oren][4], box[3], g[nbin][4];
	double duration, wn,wo,sumw[10],sumwH1,sumwH2,w[10000],wO[10000],wH1[oren],wH2[oren], utot[nmove/fout+1],upress[nmove/fout+1];
	...
	char directory_in[100] = "/Users/Ziwei/Desktop/INPUT/", open_name[100] = "mid30.gro", directory_out[100] = "/Users/Ziwei/Desktop/TEST/";
	FILE *fp_log;
	clock_t start, finish;

	start = clock();
	printf("ProName:Dooley_v2.1  Job starts\n");
	n = frand();    /*discard the first random*/
	sprintf(address_log, "%slog.txt", directory_out);
	fp_log = fopen(address_log, "w");
	fprintf(fp_log,"ProName:Dooley_v2.1  JOB STARTS\n");
...
//	}
	...
	for(n=1;n<(nmove+1);n++)
	{
		...
		for(imol=0;imol<N;imol++)         /*decide the molecules to be removed*/
			if(distance(coordinate[imol][0],center,L) < RC_Sel/10)
			{
				index_sel[nsel] = imol;
				nsel++;
			}
		...
		for(imol=0;imol<nsel;imol++)
		{
			...
			for(j=0;j<k;j++)             /*generate k trials for moleclue(oxygen)*/
			{
				...
				rand_sphere_1(center, coordinate[index_sel[imol]][0], RC_Sel/10);
				nojump(L,coordinate[index_sel[imol]][0]);							/*DECIDE THE POSITION FOR OXYGEN*/
				copy_array_3(coordinate[index_sel[imol]][0],select[j][0]);
				wO[j] =  Ueff_cav(coordinate, g, nbin, index_sel[imol], 0, N, L,index_sel, nsel, imol);
//				printf("wO[%d] = %.10f\n",j,wO[j]);
				w[j] *= wO[j];
				...
				{
					rand_sphere_2(coordinate[index_sel[imol]][0], coordinate[index_sel[imol]][1], L_OH/10);
					nojump(L,coordinate[index_sel[imol]][1]);							/*DECIDE THE POSITION FOR Hydrogen 1 */
					copy_array_3(coordinate[index_sel[imol]][1],selectH[m]);
					wH1[m] =  Ueff_cav(coordinate, g, nbin, index_sel[imol], 1, N, L,index_sel, nsel, imol);
					sumwH1 += wH1[m];
				}
				jbook = Select(wH1,sumwH1);
				copy_array_3(selectH[jbook], select[j][1]);
				copy_array_3(selectH[jbook], coordinate[index_sel[imol]][1]);
				w[j] *= wH1[jbook];

				sumwH2 = 0;
				for(m=0;m<oren;m++)             /*generate oren trials for hydrogen 2 */
				{
					gen_H2(coordinate[index_sel[imol]], L);
					nojump(L,coordinate[index_sel[imol]][2]);							/*DECIDE THE POSITION FOR Hydrogen 2 */
					copy_array_3(coordinate[index_sel[imol]][2],selectH[m]);
					wH2[m] = Ueff_cav(coordinate, g, nbin, index_sel[imol], 2, N, L,index_sel, nsel, imol);
					sumwH2 += wH2[m];
				}
			...
			printf("sumw = %.10f\n",sumw[imol]);
			fprintf(fp_log,"sumw = %.10f\n",sumw[imol]);
			printf("select:w[%d] = %.10f\n",jchoose,w[jchoose]);
			fprintf(fp_log,"select:w[%d] = %.10f\n",jchoose,w[jchoose]);

			wn *= sumw[imol];    /*Calculate the W(new)*/
			...
			for(j=0;j<k;j++)          /*generate k-1 trials*/
			{
				...
				if(j != 0)
				{
					...
					jbook = Select(wH2,sumwH2);
					w[j] *= wH2[jbook];
					fprintf(fp_log,"old: w[%d] = %.10f\n",j,w[j]);
				}
				else
				{
					w[j] *=  Ueff_cav(temp, g, nbin, index_sel[imol], 0, N, L,index_sel, nsel, imol);
					w[j] *=  Ueff_cav(temp, g, nbin, index_sel[imol], 1, N, L,index_sel, nsel, imol);
					w[j] *=  Ueff_cav(temp, g, nbin, index_sel[imol], 2, N, L,index_sel, nsel, imol);
//					printf("%dth old w[0]  %.20f\n",imol,w[j]);
					fprintf(fp_log,"%dth old w[0]  %.20f\n",imol,w[j]);
			...
			fprintf(fp_log,"sumw = %.10f\n",sumw[imol]);
			wo *= sumw[imol];			 /*Calculate the W(old)*/

			copy_array_2(select[jchoose], coordinate[index_sel[imol]]);

			printf("new usite:%.3f\n",Usite_cav(coordinate, index_sel[imol],  0, N, L, index_sel, nsel, imol)+Usite_cav(coordinate, index_sel[imol],  1, N, L, index_sel, nsel, imol) +Usite_cav(coordinate, index_sel[imol],  2, N, L, index_sel, nsel, imol));
			fprintf(fp_log,"new usite:%.3f\n",Usite_cav(coordinate, index_sel[imol],  0, N, L, index_sel, nsel, imol)+Usite_cav(coordinate, index_sel[imol],  1, N, L, index_sel, nsel, imol) +Usite_cav(coordinate, index_sel[imol],  2, N, L, index_sel, nsel, imol));
		}
		for(imol=0;imol<nsel;imol++)
		{
			printf("new usite final:%.3f\n",Usite(coordinate, index_sel[imol],  0, N, L)+Usite(coordinate, index_sel[imol],  1, N, L) +Usite(coordinate, index_sel[imol],  2, N, L));
			fprintf(fp_log,"new usite final:%.3f\n",Usite(coordinate, index_sel[imol],  0, N, L)+Usite(coordinate, index_sel[imol],  1, N, L) +Usite(coordinate, index_sel[imol],  2, N, L));
		}
		...
		for(imol=0;imol<nsel;imol++)
			for(isite=0;isite<3;isite++)
			{
				uact_n += Usite_cav(coordinate, index_sel[imol], isite, N, L,index_sel, nsel, imol);
				uact_o += Usite_cav(temp, index_sel[imol], isite, N, L,index_sel, nsel, imol);
				ueff_n *= Ueff_cav(coordinate, g, nbin, index_sel[imol], isite, N, L,index_sel, nsel, imol);
				ueff_o *= Ueff_cav(temp, g, nbin, index_sel[imol], isite, N, L,index_sel, nsel, imol);
			}
		...
		fprintf(fp_log,"wn:%.3f	",wn);
		fprintf(fp_log,"wo:%.3f	",wo);
		fprintf(fp_log,"wn/wo:%.3f	",(wn/wo));
