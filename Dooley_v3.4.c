...
//use the shell model, insertion of water in outer shell are  based on bias potential, while inner shell are also based on bias potential
//for bias potential, also use the adf of angle OOH for Oxygen and Hydrogen beside the rdf.
//Ziwei Guo  Jul-22-2015
...
void cal_ang_OOH(double coordinate[][3][4], int imol, int jmol, int N, int L, double costheta[])  /*used for decide the bias for oxygen*/
{
...
void cal_ang_OOH_2(double coordinate[][3][4], int imol, int jmol, int N, int L, double costheta[], double id_H)  /*used for decide the bias for hydrogen*/
{
...
{															/*the same as Ueff, but consider the inserion order in a cavity*/
	int tmol, tsite, s;
	double ueff=1.0, r, rc,costheta[2];
	for(tmol=0;tmol<N;tmol++)
	{
...
int Select(double w[], double sumw)  /*select one based on Boltzman weight*/
{
	int n;
	double ws,cumw;
	ws = frand()*sumw;
...
int main(void)
{
	 ...
	 */
	int N = 4913, nmove = 1000, fout = 10, k = 1000,oren = 50, acc_count = 0, natom = 0, num_cps = 0, num_shell = 0, nbin_rdf = 1600, nbin_adf = 750;  /*k:number of positions for O, oren:number of positions for H*/
	...
	double L = 5.27731, T = 300.0, beta, coordinate[N][3][4], temp[N][3][4], center[3],select[k][3][4], selectH[oren][4],box[3],cps[N][3][4],temp_cps[N][3][4], g[nbin_rdf][4], a[nbin_adf+1][2]; /*cps used to store the molecules in cavity + surrounding*/
	...
	char directory_in[100] = "/Users/Ziwei/Desktop/INPUT/", open_name[100] ="w100.gro", directory_out[100] = "/Users/Ziwei/Desktop/jul_23_001/";
	FILE *fp_log;
	...
	beta = 1.0/(1.3806488*pow(10,-23)*T);  /*unit for Boltzman constant is J/K */
		...
		for(imol=0;imol<N;imol++)
		{
			...
			if(r < (RC_Sel_out+RC_COULOMB)/10)            /*use RC_Sel+RC_COULOMB to define the cutoff of the surrounding molecules around a cavity*/
			{
				...
				if(r < RC_Sel_out/10)
				{
					...
					nsel++;
					if(r > RC_Sel_in/10)
						num_shell += 1;    /*judge a molecule is in a inside shell or outside*/
				}
			}
		}
...
//					Told *= Ueff_cav(temp_cps, g, a, nbin_rdf, nbin_adf, index_sel_cps[imol], isite, num_cps, L,index_sel_cps, nsel, imol);
		...
		if(wo-0<0.0000000001 || ueff_n-0<0.0000000001 || wn/wo-0<0.0000000001)
			acc = 0;
