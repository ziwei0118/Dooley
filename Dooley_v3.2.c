...
//for eff potential, also use the adf of angle OOH beside the rdf.
//Ziwei Guo  Jul-07-2015
...
void cal_ang_OOH(double coordinate[][3][4], int imol, int jmol, int N, int L, double costheta[])
{
...
double Ueff(double coordinate[][3][4], double g[][4], double a[][2], int nbin_rdf, int nbin_adf,int imol, int isite, int N, double L)  /*calculate corresponding Ueff(for one site) based on rdf */
{
	int tmol, tsite;
	...
	for(tmol=0;tmol<N;tmol++)
	{
		...
		else
		{
			for(tsite=0;tsite<3;tsite++)
			{
				...
				if(isite == 0 && tsite == isite && rc < RC_Adf/10 )
				{
					cal_ang_OOH(coordinate, tmol, imol, N, L, costheta);
...
int main(void)
{
	...
	int N = 4913, nmove = 5, fout = 5, k = 1000,oren = 50, acc_count = 0, natom = 0, num_cps = 0, num_shell = 0, nbin_rdf = 1600, nbin_adf = 750;
	...
	char directory_in[100] = "/Users/Ziwei/Desktop/INPUT/", open_name[100] ="w100.gro", directory_out[100] = "/Users/Ziwei/Desktop/V1/";
	FILE *fp_log;
	clock_t start, finish;

	start = clock();
	printf("ProName:Dooley_v3.2  Job starts\n");
	n = frand();    /*discard the first random*/
	sprintf(address_log, "%slog.txt", directory_out);
	fp_log = fopen(address_log, "w");
	fprintf(fp_log,"ProName:Dooley_v3.2     JOB STARTS\n");
	...
	for(n=1;n<(nmove+1);n++)
	{
		...
		for(imol=0;imol<nsel;imol++)
		{
			...
			for(j=0;j<k;j++)             /*generate k trials for moleclue(oxygen)*/
			{
				if(imol < num_shell)
				{
					...
					if(wO[j]-0 < 0.0000000001)    /*if wO is too small, we do not need to calculate hydrogen*/
						continue;
					else
					{
					sumwH1 = 0;
		...
		if(num_shell > 0 && terminate == 0)
		{
			...
			for(imol=0;imol<num_shell;imol++)
				for(isite=0;isite<3;isite++)
				{
					...
					ueff_n *= Ueff_cav(cps, g, a, nbin_rdf, nbin_adf, index_sel_cps[imol], isite, num_cps, L,index_sel_cps, nsel, imol);
					ueff_o *= Ueff_cav(temp_cps, g, a, nbin_rdf, nbin_adf, index_sel_cps[imol], isite, num_cps, L,index_sel_cps, nsel, imol);
				}
		...
		printf("n_sel:%d   n_shell:%d	",nsel, num_shell);
		printf("wn:%.20f	",wn);
		printf("wo:%.20f	",wo);
		...
		fprintf(fp_log,"n_sel:%d   n_shell:%d	",nsel, num_shell);
