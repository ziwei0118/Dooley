...
int main(void)
{
	...
	int N = 4913, nmove = 5000, fout = 50, k = 1000,oren = 10, acc_count = 0, natom = 0, num_cps = 0;
	int n, j,m, jbook, jchoose, nsel, imol=0, isite=0;         /*loop control*/
	int index_sel[N],index_cps[N], index_sel_cps[N];
	double L = 5.27731, T = 300.0, beta, coordinate[N][3][4], temp[N][3][4], center[3],select[k][3][4], selectH[oren][4],box[3],cps[N][3][4],temp_cps[N][3][4]; /*cps used to store the molecules in cavity + surrounding*/
	...
	char directory_in[100] = "/Users/Ziwei/Desktop/INPUT/", open_name[100] ="w100.gro", directory_out[100] = "/Users/Ziwei/Desktop/r4.0/";
	FILE *fp_log;
	clock_t start, finish;

	start = clock();
	printf("ProName:Dooley_v2.0  Job starts\n");
	n = frand();    /*discard the first random*/
	sprintf(address_log, "%slog.txt", directory_out);
	fp_log = fopen(address_log, "w");
	fprintf(fp_log,"ProName:Dooley_v2.0  JOB STARTS\n");
	fprintf(fp_log,"%d trials will performed on %d molecules at %.2fK\n\n",nmove,N,T);
	fprintf(fp_log,"k = %d, oren = %d, r_cav = %.2fA\n",k,oren,RC_Sel);
	...
	for(n=1;n<(nmove+1);n++)
	{
		...
		for(imol=0;imol<N;imol++)
		{
			...
			if(r < (RC_Sel+RC_COULOMB)/10)            /*use RC_Sel+RC_COULOMB to define the cutoff of the surrounding molecules around a cavity*/
			{
		...
		for(imol=0;imol<nsel;imol++)
		{
			...
			for(j=0;j<k;j++)             /*generate k trials for moleclue(oxygen)*/
			{
				...
				rand_sphere_1(center, cps[index_sel_cps[imol]][0], RC_Sel/10);
...
//				fprintf(fp_log,"newO: wO[%d] = %.10f\n",j,wO[j]);
				...
				for(m=0;m<oren;m++)             /*generate oren trials for hydrogen 1 */
				{
					...
					sumwH1 += wH1[m];
//					fprintf(fp_log,"newH1: wH1[%d] = %.10f\n",m,wH1[m]);
//					fprintf(fp_log,"newH1_ooo: wH1[%d] = %.10f\n",m,Usite_cav(coordinate, index_sel[imol], 1, N, L, index_sel, nsel,imol));
				}
				...
				for(m=0;m<oren;m++)             /*generate oren trials for hydrogen 2 */
				{
					...
					sumwH2 += wH2[m];
//					fprintf(fp_log,"newH2: wH2[%d] = %.10f\n",m,wH2[m]);
				}
